...
class MetaheuristicOptimizer(BaseKernelOptimizer):
    def __init__(
        ...
        def cost(array):
            array = array.ravel()
            the_array = np.concatenate(
                [array, np.array([initial_kernel.type])]
            )
            # create kernel
            the_kernel = Kernel.from_numpy(
                the_array,
            ...
            print(
                f"MetaheuristicOptimizer.cost -> {the_cost: 5.5f} -> {array}"
            )
            return the_cost
        ...
        self.opt = Opytimizer(
            self.space, self.optimizer, self.function, save_agents=True
        )
        self.history = None
    ...
    def optimize(self, n_iterations=1000, verbose=False):
        self.opt.start(
            ...
            callbacks=[CustomCallback()] if verbose else [],
        )
        ...
        the_array = np.concatenate(
            [self.best_solution, np.array([self.initial_kernel.type])]
        )
    ...
    def get_opytimize_space(self):
        ...
        upper_bound = [
            len(allowed_generators) - 1,
            n_qubits - 1,
            n_qubits - 2,
            n_features,
            1.0,
        ] * n_operations + [3] * n_qubits
