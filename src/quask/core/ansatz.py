...
class Ansatz:
    ...
    def __init__(
        ...
        assert n_features > 0, (
            "Cannot have zero or negative number of features"
        )
        assert n_operations > 0, (
            "Cannot have zero or negative number or operations"
        )
    ...
    def change_bandwidth(self, operation_index: int, new_bandwidth: float):
        ...
        :return: None
        """
        assert 0 <= operation_index < self.n_operations, (
    ...
    def change_generators(self, operation_index: int, new_generator: str):
        ...
        assert new_generator in Operation.OPERATIONS, (
            f"Unknown generator {new_generator}"
        )
        if not self.allow_midcircuit_measurement:
            assert new_generator not in Operation.MEASUREMENT_OPERATIONS, (
                "Mid-circuit measurement not allowed"
            )
    ...
    def change_feature(self, operation_index: int, new_feature: int):
        ...
        assert 0 <= new_feature <= self.n_features, (
            f"Feature index out of bounds ({new_feature=})"
        )
    ...
    def change_wires(self, operation_index: int, new_wires: list[int]):
        ...
        assert len(new_wires) == 2, (
            "The location is a list of two integers, not less no more"
        )
        assert 0 <= new_wires[0] < self.n_qubits, (
            f"First wire index out of bounds ({new_wires=})"
        )
        assert 0 <= new_wires[1] < self.n_qubits, (
            f"Second wire index out of bounds ({new_wires=})"
        )
        assert new_wires[0] != new_wires[1], (
            f"Cannot specify the same wire twice ({new_wires=})"
        )
    ...
    def initialize_to_identity(self):
        ...
        for i in range(self.n_operations):
            self.operation_list[i] = Operation(
                "II", [0, 1], self.n_features, 1.0
            )
    ...
    def initialize_to_known_ansatz(self, ansatz):
        ...
        for i in range(self.n_operations):
            ...
            self.change_operation(
                i, op.feature, op.wires, op.generator, op.bandwidth
            )
    ...
    def from_numpy(
        ...
        """
        ans = Ansatz(
            n_features, n_qubits, n_operations, allow_midcircuit_measurement
        )
        ...
        for i in range(n_operations):
            ...
            wires = [
                np.rint(array[i * 5 + 1]).astype(int),
                np.rint(array[i * 5 + 2]).astype(int),
            ]
            feature = np.rint(array[i * 5 + 3]).astype(int)
            bandwidth = np.round(array[i * 5 + 4], decimals=4)
            if shift_second_wire and wires[1] >= wires[0]:
                wires[1] += 1
            ans.change_operation(
                i, feature, wires, Operation.OPERATIONS[generator], bandwidth
            )
        return ans
