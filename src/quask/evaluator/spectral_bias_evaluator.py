...
class SpectralBiasEvaluator(KernelEvaluator):
    ...
    See: Canatar, Abdulkadir, Blake Bordelon, and Cengiz Pehlevan. 
    "Spectral bias and task-model alignment explain generalization
    in kernel regression and infinitely wide neural networks."
    Nature communications 12.1 (2021): 2914.
    """

    def __init__(self, n_eigenvalues_cut):
        """Initialization.

        :param n_eigenvalues_cut: number of eigenvalues contributing to the 
            cumulative power
        """
        super().__init__()
    ...
    def evaluate(
        ...
        C, powers = SpectralBiasEvaluator.cumulative_power_distribution(
            w, Lambda, self.n_eigenvalues_cut
        )
    ...
    def decompose_kernel(
        K, eigenvalue_descending_order=True, eigenvalue_removal_threshold=1e-12
    ):
        """Decompose kernel.

        Decompose the kernel matrix K in its eigenvalues Λ
        and eigenvectors Φ.
        ...
        # kernel matrix is positive definite, any (small) negative eigenvalue 
        # is effectively a numerical error
        Lambda[Lambda < 0] = 0
    ...
    def calculate_weights(Lambda, Phi, labels):
        ...
        Calculates the weights of a predictor given the labels and the kernel 
        eigendecomposition, as shown in (Canatar et al 2021, 
        inline formula below equation 18).

        :param Lambda: vectors of m nonnegative eigenvalues 'eta'
        :param Phi: vectors of m nonnegative eigenvectors 'phi'
        :param labels: vector of m labels corresponding to 'm' ground truth 
            labels or predictor outputs
        ...
        # weight vectors are calculated by inverting formula: 
        #     y = \sum_k=1^M w_k \sqrt{lambda_k} \phi_k(x)
