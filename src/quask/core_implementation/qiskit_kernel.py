...
from qiskit_ibm_runtime import IBMBackend, QiskitRuntimeService, RuntimeJobV2
...
class QiskitKernel(Kernel):
    ...
    def get_sampler_options(self):
        sampler_options = sop(
            ...
            dynamical_decoupling={
                "enable": bool(
                    self.options["dynamical_decoupling"]["sequence_type"]
                ),
                "sequence_type": self.options["dynamical_decoupling"][
                    "sequence_type"
                ],
            },
            twirling={
    ...
    def kappa(self, x1: np.ndarray, x2: np.ndarray) -> float:
        ...
        if self.type == KernelType.OBSERVABLE:
            ...
            qc.append(
                self.get_qiskit_ansatz().assign_parameters(x1.tolist()),
                range(self.ansatz.n_qubits),
            )
            qc.append(
                self.get_qiskit_ansatz()
                .assign_parameters(x2.tolist())
                .inverse(),
                range(self.ansatz.n_qubits),
            )
    ...
    def phi(self, x: np.ndarray) -> float:
        if self.type == KernelType.OBSERVABLE:
            ...
            raise ValueError(
                "Unknown type, possible erroneous loading from a numpy array"
            )
