...
def AnsatzTemplate(ansatz: Ansatz, params: np.ndarray, wires: np.ndarray):
    for operation in ansatz.operation_list:
        if "M" not in operation.generator:
            feature = (
                np.pi
                if operation.feature == ansatz.n_features
                else params[operation.feature]
            )
            qml.PauliRot(
                operation.bandwidth * feature,
                operation.generator,
                wires=wires[operation.wires],
            )
...
class PennylaneKernel(Kernel):
    ...
    def __init__(
        ...
        measurement_wires = np.array(
            [i for i in wires if measurement[i] != "I"]
        )
